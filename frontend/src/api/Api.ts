 ...
 * @param codeBlockDTO
 */
export const getRandomCodeBlock: (
  codeBlockDTO: CodeBlockDTO
) => Promise<AxiosResponse<CodeBlockResponse>> = async (
  codeBlockDTO: CodeBlockDTO
) => {
 ...
 */
export const getRandomAvatar: () => Promise<
  AxiosResponse<AvatarResponse>
> = async () => {
...
export const getUser: (
  token: string
) => Promise<AxiosResponse<UserProfile>> = async (token: string) => {
 ...
 * @param token
 */
export const postSoloMatchHistoryResults = async (
  createMatchHistoryItem: CreateMatchHistoryItem,
  token: string
) => {
  return await axios.post(
    `${API_ENDPOINT}/match-history/solo`,
    createMatchHistoryItem,
    {
      headers: {
        Authorization: `bearer ${token}`,
...
export const getCodeBlock: (
  codeBlockId: string
) => Promise<AxiosResponse<{ codeBlock: CodeBlock }>> = async (
  codeBlockId: string
) => {
