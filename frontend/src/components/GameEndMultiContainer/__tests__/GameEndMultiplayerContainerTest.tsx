...
describe("GameEndMultiplayerContainer", () => {
  ...
  it("should render the container for the end screen results", () => {
    const {baseElement} = render(<GameEndMultiplayerContainer players={[]} isViewCode= {true} codeBlock= {codeBlock} />);
  ...
  it("should correctly render code block", () => {
    const {getByText} = render(<GameEndMultiplayerContainer players={[]} isViewCode= {true} codeBlock= {codeBlock}/>);

    expect(getByText("test code")).toBeInTheDocument();
  });

  it("should correctly render the players in the end game screen", () => {
    const {getByText} = render(<GameEndMultiplayerContainer players={dummyPlayers} isViewCode= {false} codeBlock= {codeBlock}/>);
