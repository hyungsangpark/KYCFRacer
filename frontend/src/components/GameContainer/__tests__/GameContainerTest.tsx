...
describe("GameContainer", () => {
  ...
  it("should render", () => {
    const {baseElement} = render(<GameContainer started={true} onGameOver={jest.fn} totalGameTimeInSeconds={30} code="test code" updateStats={jest.fn} language={"javascript"}/>);

    expect(baseElement).toBeInTheDocument();
  });

  it("if not started it should have a text of passed in timer", () => {
    const {getByText} = render(<GameContainer started={false} onGameOver={jest.fn} totalGameTimeInSeconds={30} code="test code" updateStats={jest.fn} language={"javascript"}/>);

    expect(getByText("Time: 00:30")).toBeInTheDocument();
  });

  it("if started then it should have a 3 second timer", () => {
    const {getByText} = render(<GameContainer started={true} onGameOver={jest.fn} totalGameTimeInSeconds={30} code="test code" updateStats={jest.fn} language={"javascript"}/>);
