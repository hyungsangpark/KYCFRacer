...
describe("LobbyPlayerContainer", () => {
  ...
  it("if players of length 0 is passed in then a loading screen should be rendered", () => {
    ...
    expect(baseElement.querySelector("[data-testid='loading-screen']")).toBeInTheDocument();
  })

  it("if players of length more than 1 is passed in then main screen is rendered", () => {
    const {baseElement} = render(<LobbyPlayerContainer players={[dummyPlayers[0]]}/>);

    expect(baseElement.querySelector("[data-testid='main-screen']")).toBeInTheDocument();
  })

  it("it renders every player correctly", () => {
    const {getByText} = render(<LobbyPlayerContainer players={dummyPlayers}/>);
  ...
  it("showStats and includeNumbers correctly render", () => {
    const {getByText} = render(<LobbyPlayerContainer players={[dummyPlayers[0]]} includeNumbers={true} showStats={true}/>);
